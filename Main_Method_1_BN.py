...
def P_bn(z, is_training):
    ...
    h1 = tf.contrib.layers.batch_norm(h1, center=True, scale=True, is_training=is_training, scope='bn_1')
    h1 = tf.nn.relu(h1)
	#
    ...
    h2_1 = tf.contrib.layers.batch_norm(h2_1, center=True, scale=True, is_training=is_training, scope='bn_21')
    ...
    h2_2 = tf.contrib.layers.batch_norm(h2_2, center=True, scale=True, is_training=is_training, scope='bn_22')
    h2_2 = tf.nn.relu(h2_2)
	#
    h3_1 = tf.add(tf.nn.conv2d_transpose(h2_2, deconv3_1_weight, strides=[1, 1, 1, 1], padding='SAME',
                                                    output_shape=[batch_size, width / 4, height / 4,
                                                                  deconv3_1_features]), deconv3_1_bias)
    h3_1 = tf.contrib.layers.batch_norm(h3_1, center=True, scale=True, is_training=is_training, scope='bn_31')
    h3_1 = tf.nn.relu(h3_1)
	#
    h3_2 = tf.add(tf.nn.conv2d_transpose(h3_1, deconv3_2_weight, strides=[1, 2, 2, 1], padding='SAME',
                                                    output_shape=[batch_size, width / 2, height / 2,
                                                                  deconv3_2_features]), deconv3_2_bias)
    h3_2 = tf.contrib.layers.batch_norm(h3_2, center=True, scale=True, is_training=is_training, scope='bn_32')
    h3_2 = tf.nn.relu(h3_2)
	#
    h4_1 = tf.add(tf.nn.conv2d_transpose(h3_2, deconv4_1_weight, strides=[1, 1, 1, 1], padding='SAME',
                                                    output_shape=[batch_size, width / 2, height / 2,
                                                                  deconv4_1_features]), deconv4_1_bias)
    h4_1 = tf.contrib.layers.batch_norm(h4_1, center=True, scale=True, is_training=is_training, scope='bn_41')
    h4_1 = tf.nn.relu(h4_1)
	#
    h4_2 = tf.add(tf.nn.conv2d_transpose(h4_1, deconv4_2_weight, strides=[1, 2, 2, 1], padding='SAME',
                                                    output_shape=[batch_size, width / 1, height / 1,
                                                                  deconv4_2_features]), deconv4_2_bias)
    h4_2 = tf.contrib.layers.batch_norm(h4_2, center=True, scale=True, is_training=is_training, scope='bn_42')
    ...
    h5 = tf.contrib.layers.batch_norm(h5, center=True, scale=True, is_training=is_training, scope='bn_5')
	#
...
while len(index_ind) <= terminate_step:
    ...
    for it in range(100000):
        ...
        if it%100 == 0:
			...
			final_error=0
			for it_0 in range(ratio):
				final_error_temp=sess.run(recon_loss, feed_dict={y_output:Y_test[it_0%ratio*batch_size:it_0%ratio*batch_size+batch_size].T,
				                                                            F_input:test_load[it_0%ratio*batch_size:it_0%ratio*batch_size+batch_size],
				                                                            is_training: True})
